...
2. **Upload Files to Storage:** Once the storage is set up, users can proceed to upload files containing domain-specific knowledge into the configured storage system. Casibase supports multiple document formats including `.txt`, `.md`, `.docx`, and `.pdf`. For enhanced parsing of Word documents, Casibase can optionally leverage [markitdown](https://github.com/microsoft/markitdown) for better text extraction and format handling.
