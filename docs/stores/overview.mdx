...
Stores in Casibase can be configured to use vectors from other stores through the **Vector stores** field. This powerful feature allows a single store to search for knowledge across multiple knowledge bases, enabling:

- **Unified Knowledge Access**: A main store can access vectors from multiple specialized stores
- **Knowledge Base Composition**: Combine domain-specific knowledge bases for comprehensive search results
- **Flexible Information Retrieval**: Each store can dynamically access relevant knowledge from other stores without data duplication
