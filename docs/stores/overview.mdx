...
### 2.5 Flexible Interface Customization

The store list interface adapts to different workflows. When using stores primarily for file tree management rather than AI chat, the "Hide chat" toggle simplifies the view by hiding AI-related columns. Stores also support example questions that guide users when starting new conversations, making it easier for users to discover what each store can do.
