...
When configuring providers, authentication credentials are protected with enhanced security features. The API key field (used for provider authentication) is masked by default to prevent credential exposure. You can toggle the visibility icon to reveal or hide the key content when needed. Provider keys, which identify your provider configuration in Casibase, also use password masking for added security.
