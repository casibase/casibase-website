...
Scan Providers enable security and network scanning capabilities for infrastructure assets. They integrate with scanning tools to perform vulnerability assessments and network discovery on virtual machines and other assets. Casibase currently supports Nmap for network exploration and security scanning, with structured JSON output for easy analysis of scan results.
