...
This command starts a local Ethereum node with HTTP JSON-RPC enabled and sets the CORS domain to allow cross-origin requests from `https://remix.ethereum.org`. This configuration is suitable for online contract deployment and interaction using Remix or other web-based tools. You can use the default account (private key can be found in the `geth` console at first launch) and the endpoint (`http://127.0.0.1:8545`) as your Provider URL in Casibase for immediate testing and development.

<img src="/img/blockchain/ethereum/configuration-provider.png" alt="Geth Dev Mode Configuration" width="600"/>

When you run Geth in dev mode for the first time (without any existing data), the console will display important information as shown in the image above:

- **Private Key**: The console shows the private key of the pre-funded account that you can use for testing. This key is automatically generated and displayed only during the first run.
...
### 2.3 Contract Address and Invoke Method Configuration
...
To deploy a smart contract on Ethereum, you can refer to the [Casibase/contract-storage-eth](https://github.com/casibase/contract-storage-eth). This repository provides sample Solidity contracts and deployment scripts using Go and Remix. After deployment, you can obtain the contract address from the deployment output.
